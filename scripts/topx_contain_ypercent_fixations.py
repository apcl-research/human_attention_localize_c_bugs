...
def calculate_top_units(data, percentage, unit, count_col_name, curr_data=None):
    ...
    return num_units, data_sorted, curr_data
...
def plot_fixation_distribution(data, bug, participant, percentage, output_dir, official_bug_names, unit, count_col_name, unit_col_name):
    ...
    num_units, sorted_units, curr_data = calculate_top_units(data, percentage, unit, count_col_name, curr_data)
...
def plot_bug_summary(bug_data, bug, percentage, output_dir, official_bug_names, unit, count_col_name, unit_max, y_cutoff=150):
    ...
    for participant, data in bug_data.items():
        num_top, sorted_units, _ = calculate_top_units(data, percentage, unit, count_col_name)
    ...
    def draw_break_marker(ax, x, width, y_cutoff):
        ...
        ax.plot([x_start, x_start + marker_width],[y_start, y_start + marker_slope],color='black', lw=0.5, clip_on=False)
        ax.plot([x_start, x_start + marker_width],[y_start + 2, y_start + 2 + marker_slope],color='black', lw=0.5, clip_on=False)
    ...
    for ax, (participant, sorted_units) in zip(axes.flatten(), participant_sorted_units_items):
        ...
        if total_units > unit_max:
            ...
            labels = [f"f{i+1}" for i in range(unit_max)] + [f"f{unit_max}-{total_units}"]
        else:
        ...
        # Set y-axis limit so that bars taller than y_cutoff are truncated
        ...
        # For bars that exceed y_cutoff, add break markers and annotate the true value
        for bar in bars:
            ...
            if height > y_cutoff:
                ...
                width = bar.get_width()
                if unit == "functions":
                    draw_break_marker(ax, x, width, y_cutoff)
                if x == 1: 
                    add = width *1.6
                else: 
                    add = 0 
                ax.text(x + width/2 + add, y_cutoff + y_cutoff*0.25, f'{int(height)}',
                        ha='center', va='top', rotation=60)
                #ax.text(x + width/2, y_cutoff + y_cutoff*0.12, f'{int(height)}',
                        #ha='center', va='top', fontsize=12, rotation=60)
        
        ax.set_title(f'{participant}: Top {adjusted_top}/{total_units} {unit}', pad=23)
    ...
    plot_filename = f'{bug}_{official_bug_name}_{unit}_summary_{percentage}percent'
    plt.draw() 
    fig.savefig(os.path.join(output_dir, plot_filename+'.png'))
    fig.savefig(os.path.join(output_dir, plot_filename+'.svg'))
    fig.savefig(os.path.join(output_dir, plot_filename+'.pdf'))
    plt.close()
